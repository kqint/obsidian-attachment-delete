...
`Attachment Delete` 是一款 Obsidian 的**右键附件删除**插件。
...
- **右键一键删除**：在笔记中的附件链接（图片、视频、音频等）上点击右键，即可看到红色的“**删除附件**”选项。
- **全库引用检测**：在物理删除前，自动检索全库是否有该笔记和其他笔记引用此附件，如果有，仅移除链接而保留物理文件并提示，防止误删。
- **暂时不支持**：不支持“白板”（ `Canvas` ）中的右键菜单；在笔记中不支持 PDF 预览窗格的更多选项（3个点）菜单。

<img src="./README.assets/删除附件示例图片.png" alt="删除附件示例图片" style="zoom:50%;" />
...
- **透明反馈**：清理成功后，通知弹窗会明确列出**被删除的文件夹名称**。
...
**A:** 请检查当前光标是否悬停在**图片、音频或视频**等附件链接上。
